...
// Promotion d'un pion � la position donn�e avec le choix sp�cifi�
void PieceManager::promotePawn(Position pos, Piece::Color color, Board& board, char choice) {
	...
	if (c == 'Q') newPiece = std::make_unique<Queen>(color);
		...
		// choix invalide : ne modifie pas le plateau (appelant peut g�rer erreur)
		return;
	}
	board.changePiece(pos, std::move(newPiece));
}
// Promotion d'un pion � la position donn�e en demandant � l'utilisateur
void PieceManager::promotePawn(Position pos, Piece::Color color, Board& board) {
	...
	while (true) {
		...
		char c = input[0];
		promotePawn(pos, color, board, c); // r�utilise la version testable
		break;
	}
}
